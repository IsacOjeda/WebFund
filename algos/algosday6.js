...
function listPokemansWithIdDivisibleBy3(pokemans) {
     ...
     * }
     */
    for (var p of pokemans.filter(isMultipleOfThree)) {
        console.log(p);
    }
}
...
function listMultiTypePokemans(pokemon) {
     ...
     * }
     */
    for (var p of pokemon.filter(hasMultipleTypes)) {
        console.log(p);
    }
}
...
function listOnlyPoisonPokemans(pokemon) {
    for (var p of pokemon.filter(isOnlyPoison)) {
        console.log(p);
    }
}
...
function listPokemonWithSecondTypeFlying(pokemon) {
    ...
    for (var p of pokemon.filter(isFlyingSecond)) {
        console.log(p)
    }
}
...
function reversedNamesOfPoisonPokemon(pokemon) {
    for (var name of pokemon.filter(isOnlyPoison).map(p => p.name)) {
